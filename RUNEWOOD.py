...
player_rect = player_sprite.get_rect(topleft = (450, 350))
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if battle =="lost" and event.type == pygame.KEYDOWN or battle =="lost" and event.type == pygame.MOUSEBUTTONDOWN:
            ...
            #exit()
        if runewood == True:
            if recharge_rect.colliderect(player_rect):
            ...
            if linos_rect.colliderect(player_rect):
                if event.type == pygame.KEYDOWN:
            ...
            if nyx_shop_rect.colliderect(player_rect):
                if event.type == pygame.KEYDOWN:
            ...
            if drunken_d_rect.colliderect(player_rect):
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_e:
                        tavern = "open"
                        runewood = None
        ...
        if tavern == "open":
            if welcome_rect.colliderect(player_rect):
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_e:
                        tavern = None
                        runewood = True
    ...
    if battle is not None and enemy is not None:        #maybe health and sprite for player should stay active when exp and defeat screen
        ...
        #linos shop                                                                                                                 !!!!!
        #wallet = font1.render(f"Gold: {player.gold}", False, "White")
    ...
    if runewood == True:
        ...
        if linos_rect.colliderect(player_rect) and linos_shop is None:
            interact_e(font1, screen)
        if nyx_shop_rect.colliderect(player_rect) and linos_shop is None:
            interact_e(font1, screen)
        if drunken_d_rect.colliderect(player_rect) and linos_shop is None:
            interact_e(font1, screen)
        ...
        if player_rect.right >= 900: player_rect.right = 900
        if player_rect.top <= 0: player_rect.top = 0
        if player_rect.bottom >= 700: player_rect.bottom = 700
